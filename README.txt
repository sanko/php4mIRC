...
DESCRIPTION
    ...
    Inspired by the TCL4mIRC[1] and Python4mIRC[2] projects, PHP4mIRC is a
    DLL for the mIRC chat client[3] that allows a scripter to execute PHP
    scripts from mIRC's edit box and in mIRC's msl script files.

REQUIREMENTS

    any version of mIRC (developed on v7.1)
    PHP (developed and built for v5.3.3 nts)
...
INSTALLATION
    ...
    mIRC's directory and type...
        ...
        /load -rs C:\[path\to\mirc]\php.mrc

    ...to load the script file. This will run /php_test to test the
    installation.

USAGE & EXAMPLES

    Use /php <php syntax> to execute PHP code. Several examples of this and
    the embedded syntax are in php.mrc. I'll explain some of the nifty bits
    here...

  STDOUT/STDERR

    Somewhere near the top of php.mrc, you should see the following lines:

        ; Standard input/output handling
        on *:SIGNAL:PHP_STDOUT:if ($1 != $null) echo -a $1-
        on *:SIGNAL:PHP_STDERR:if ($1 != $null) echo $color(info) -a $1-

    Instead of redirecting all IO to the status window, php4mIRC redirects
...
{TODO
  Inline Snippets
    ...
    Midway through php.mrc, you come upon the following alias...
...
RELEASE INFORMATION
    ...
    For future updates, check http://github.com/sanko/php4mirc/ or the
    project's website http://sankorobinson.com/php4mirc/.
